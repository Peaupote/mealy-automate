...
\begin{frame}
  ...
  \begin{figure}[h!]
    ...
    \begin{subfigure}[b]{0.35\textwidth}
      \centering
      \begin{tikzpicture}
        \draw [->,>=latex] (-0.5, 1) node[left] {$p_0p_1$} to (0.5, 1) node[right] {$q_0q_1$};
        \draw [->,>=latex] (0, 1.5) node[above] {$x$} to (0, 0.5) node[below] {$y$};
      \end{tikzpicture}
...
\begin{frame}
  ...
  \begin{figure}[!ht]
    \begin{center}
      \begin{tikzpicture}
        \coordinate (P1) at (0.5,0.1);
        \coordinate (P2) at (2.5,0.1);
        \coordinate (Q1) at (5,0.1);

        \coordinate (00) at (1.3,1);
        \coordinate (01) at (3.55,1);

        \coordinate (10) at (1.3,-1);
        \coordinate (11) at (3.55,-1);
...
\begin{frame}
  ...
  Le but du projet était d'essayer d'étendre ce résultat à des classes d'automates plus grandes.
...
\begin{frame}
  ...
  \begin{figure}[!ht]
    ...
    \caption{Les plus petits contre-exemples à une généralisation du théorème. Le quatrième contre-exemple est l'automate inverse du premier. \label{fig:fantastiques}
    }
  \end{figure}
\end{frame}
  ...
  \begin{figure}[!ht]
    \begin{center}
        \scalebox{0.4}{
            ...
            \node[minimum width = 2.5em] (B) at (-3.5,0) {};

            \draw[thick,->,>=latex] (B) to[out=165,in=195,looseness=8] node[midway, left]{$(\mathfrak{mdmd})^*$} (B);
            ...
            \draw[thick,->,>=latex] (-3.5, -9) to node[midway, above] {$(\mathfrak{mdmd})^*$} (-4.5, -9);

            \coordinate (A2) at (-6, -9);
            \node[draw,thick,circle,minimum width = 2.5em] (QA2) at (A2) {};
            \draw[thick,->,>=latex] (QA2) to[out=70,in=110,looseness=6] node[midway, above]{$0|0$} (QA2);
            \end{tikzpicture}
        }
    \end{center}
  \end{figure}
...
\begin{frame}
 ...
 \begin{figure}[!ht]
 	...
 	\scalebox{0.5}{
 		...
 		\coordinate (C3) at (3,6);
 		\coordinate (D1) at (-3,4);
 		\coordinate (D2) at (0,4);
 		\coordinate (D3) at (3,4);
 		\coordinate (E1) at (-3,2);
 		\coordinate (E2) at (0,2);
 		\coordinate (E3) at (3,2);
 		\coordinate (F1) at (-3,0);
 		\coordinate (F2) at (0,0);
 		\coordinate (F3) at (3,0);
 		\node[minimum width = 2em] (QA) at (A) {$\mathcal{A}$};
 		\node[minimum width = 2em] (QB) at (B) {};
 		\node[minimum width = 2em] (QC1) at (C1) {$\mathcal{A}_1\mathcal{B}_1$};
 		\node[minimum width = 2em] (QC2) at (C2) {$\mathcal{A}_2\mathcal{B}_2$};
 		\node[minimum width = 2em] (QC3) at (C3) {$\mathcal{A}_3\mathcal{B}_3$};
 		\node[minimum width = 2em] (QD1) at (D1) {$\mathcal{B}_1\mathcal{A}_1$};
 		\node[minimum width = 2em] (QD2) at (D2) {$\mathcal{B}_2\mathcal{A}_2$};
 		\node[minimum width = 2em] (QD3) at (D3) {$\mathcal{B}_3\mathcal{1}_3$};
 		\node[minimum width = 2em] (QE1) at (E1) {};
 		\node[minimum width = 2em] (QE2) at (E2) {};
 		\node[minimum width = 2em] (QE3) at (E3) {};
 		\node[minimum width = 2em] (QF1) at (F1) {};
 		\node[minimum width = 2em] (QF2) at (F2) {};
 		\node[minimum width = 2em] (QF3) at (F3) {};
 		
 		\draw[thick,->,>=latex] (QA) to node[midway, right] {$(\mathfrak{mdmd})^*$} (QB);
 		\draw[thick,->,>=latex] (QB) to node[midway, above left] {} (QC1);
 		\draw[thick,->,>=latex] (QB) to node[midway, right] {} (QC2);
 		\draw[thick,->,>=latex] (QB) to node[midway, above right] {} (QC3);
 		\draw[thick,->,>=latex] (QC1) to node[midway, right] {$\mathfrak{c}$} (QD1);
 		\draw[thick,->,>=latex] (QC2) to node[midway, right] {$\mathfrak{c}$} (QD2);
 		\draw[thick,->,>=latex] (QC3) to node[midway, right] {$\mathfrak{c}$} (QD3);
 		\draw[thick,->,>=latex] (QD1) to node[midway, right] {$(\mathfrak{mdmd})^*$} (QE1);
 		\draw[thick,->,>=latex] (QD2) to node[midway, right] {$(\mathfrak{mdmd})^*$} (QE2);
 		\draw[thick,->,>=latex] (QD3) to node[midway, right] {$(\mathfrak{mdmd})^*$} (QE3);
 		\draw[thick,->,>=latex] (QE2) to node[midway, above left] {} (QF1);
 		\draw[thick,->,>=latex] (QE2) to node[midway, right] {} (QF2);
 		\draw[thick,->,>=latex] (QE2) to node[midway, above right] {} (QF3);
 		\end{tikzpicture}
 	}
 \end{figure}
...
\begin{frame}
  ...
  On constate que le nombre d'automates biréversibles est très grand. Pour pouvoir tous les analyser, il serait préférable de se contenter de les regarder à isomorphime près.
    ...
    \begin{center}
      \begin{tikzpicture}[->,>=latex,node distance=25mm]
        \begin{scope}[rotate=0]
          ...
          \node[draw,circle] (a2) [above of=a1] {~};%$a_2$};
          \node         (b0) [below of=a1,node distance=13mm] {};
          \node[draw,circle] (b1) [right of=b0,node distance=14mm] {~};%$b_1$};
          \node[draw,circle] (b2) [left of=b0,node distance=14mm] {~};%$b_2$};
          \node[draw,circle] (b3) [below of=b0,node distance=25mm] {~};%$b_3$};
          \path	(a1)	edge[bend left=10]		node[align=center,left]	{\(1|1\)}				(a2)
          (a2)	edge[bend left=10]		node[align=center,right]	{\(0|1\)\\\(1|0\)\\\(2|2\)}	(a1)
          (b1)	edge[bend left=10]		node[align=center,below]	{\(1|0\)}				(b2)
          (b2)	edge[bend left=10]		node[align=center,above]	{\(0|1\)}				(b1)
          (b1)	edge[bend left=10]		node[align=center,below right=-.1]	{\(2|1\)}		(b3)
          (b3)	edge[bend left=10]		node[align=center,above left=-.1]	{\(1|2\)}		(b1)
          (b2)	edge[bend left=10]		node[align=center,above right=-.1]	{\(1|2\)}		(b3)
          (b3)	edge[bend left=10]		node[align=center,below left=-.1]	{\(2|1\)}		(b2)
          (b3)	edge[loop below]		node[align=center]{\(0|0\)}						(b3);
          \path	[red]
          (a1)	edge node[align=center,pos=.35,above right=-.1]		{\(2|0\)}	(b1)
          (a1)	edge node[align=center,pos=.35,above left=-.1]			{\(0|2\)}	(b2)
          (b1)	edge[bend right=40] node[align=center,pos=.75,right]	{\(0|2\)}   (a2)
          (b2)	edge[bend left=40] node[align=center,pos=.75,left]		{\(2|0\)}	(a2);
        \end{scope}
        \begin{scope}[->,>=latex,node distance=28mm,xshift=-8cm,yshift=8mm]
          \node[draw,circle] (a1) {~};%$3$};
          \node[draw,circle] (a2) [right of=a1,node distance=36mm] {~};%$5$};
          \node         (b0) [right of=a1,node distance=18mm] {};
          \node[draw,circle] (b1) [below of=b0,node distance=10mm] {~};%$4$};
          \node[draw,circle] (b2) [below of=b1,node distance=18mm] {~};%$2$};
          \node[draw,circle] (b3) [below of=b2,node distance=18mm] {~};%$1$};
          \path	(a2)	edge[bend right=40]		node[align=center,above]	{\(0|0\)\\\(1|2\)\\\(2|1\)}	(a1)
          (b1)	edge[bend left=10]		node[align=center,right]	{\(0|1\)}				(b2)
          (b2)	edge[bend left=10]		node[align=center,left]	{\(2|0\)}				(b1)
          (b2)	edge[bend left=10]		node[align=center,right]	{\(0|2\)}				(b3)
          (b3)	edge[bend left=10]		node[align=center,left]	{\(1|0\)}				(b2)
          (b1)	edge[loop above]		node[align=center]		{\(1|2\)}				(b1)
          (b3)	edge[loop below]		node[align=center]		{\(2|1\)}				(b3);
          \path	[red]
          (a1)	edge node[align=center,above]							{\(0|1\)}	(b1)
          (a1)	edge[bend right=15] node[align=center,pos=.6,below left=-.1]	{\(2|2\)}	(b2)
          (a1)	edge[bend right=30] node[align=center,pos=.7,below left=-.1]	{\(1|0\)}	(b3)
          (b1)	edge node[align=center,above]							{\(2|0\)}   (a2)
          (b2)	edge[bend right=15] node[align=center,pos=.4,below right=-.1]	{\(1|1\)}   (a2)
          (b3)	edge[bend right=30] node[align=center,pos=.3,below right=-.1]	{\(0|2\)}   (a2);
        \end{scope}
      \end{tikzpicture}
    \end{center}
  \end{figure}

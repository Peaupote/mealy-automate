...
\begin{figure}[!ht]
  \begin{center}
    \begin{tikzpicture}
      ...
      \coordinate (00) at (1,1);
      \coordinate (01) at (3,1);
      \coordinate (02) at (5,1);

      \coordinate (10) at (1,-1);
      \coordinate (11) at (3,-1);
      \coordinate (12) at (5,-1);
      ...
      \draw[->,>=latex] (L02) -- (L12);
    \end{tikzpicture}
    \caption{action de l'état 0 de la machine $\mathcal{A}$ sur le mot 101. Sont représentés horizontalement les états successifs et verticalement l'action de chacun de ces états sur les lettres du mot.}
  \end{center}
\end{figure}

Une autre représentation qui sera commode dans la partie \ref{sec:gen} est le graphe en hélice. On représente l'automate par un graphe dont les sommets sont étiquetés par des couples état-lettre. Pour chaque flèche du type $p\overset{x|y}{\longrightarrow}q$ dans l'automate, il y a une flèche de l'état $p, x$ vers l'état $q, y$ dans le graphe en hélice. Aussi remarque-t-on que de chaque sommet du graphe en hélice part exactement une flèche.

\begin{figure}[!ht]
  \begin{center}
    \begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
      \node[state] (q_00) {$p, 0$};
      \node[state] (q_11) [right=of q_00] {$q, 1$};
      \node[state] (q_10) [above=of q_11] {$q, 0$};
      \node[state] (q_01) [above=of q_00] {$p, 1$};
      \path[->,>=latex]
      (q_00) edge node {} (q_11)
      (q_10) edge [loop right] node {} (q_10)
...
La réciproque est fausse en général (voir figure \ref{fig:fantastiques}). Il semble pourtant que dans la classe particulière des automates biréversibles, la $\mathfrak{md}$-réduction est particulièrement efficace.
...
Ce théorème n'est toutefois pas valide pour les automates biréversibles en général.
\newpage
...
\begin{figure}[!ht]
  ...
  \end{center}
  \caption{Les plus petits contre-exemples à une généralisation du théorème \ref{thm:K}. Le quatrième contre-exemple est l'automate inverse du premier. Dans la suite, nous appelons ces automates $\mathfrak{c}$-faible.\label{fig:fantastiques}
  }
\end{figure}

On observe cependant que les contre-exemples de la figure \ref{fig:fantastiques} sont factorisables et qu'en permutant leurs facteurs on trouve un automate $\mathfrak{md}$-trivial (figure \ref{fig:mdc-red-ex}). Cela conduit à introduire les opérations suivantes.
...
\end{center}
  \caption{$\mathfrak{mdc}$-réduction du deuxième contre-exemple. Il est conjugué avec l'automate $\mathcal{B}$ de la figure \ref{fig:md-red-ex} qui est $\mathfrak{md}$-trivial.\label{fig:mdc-red-ex}}
\end{figure}
\newpage
...
Cette conjecture est l'objet des recherches menées au cours de ce projet.
...
\begin{proof}
  ...
  On en déduit que tout élément de $\left<\mathcal{B}\mathcal{A}\right>$ s'écrit comme la composition d'un $\rho_{q},~q\in\mathcal{Q}_1$, d'un $\rho_\bullet\in\left<\mathcal{A}\mathcal{B}\right>$ et d'un $\rho_{p},~p\in\mathcal{Q}_2$. Or $\mathcal{Q}_1$ et $\mathcal{Q}_2$ sont finis, donc le nombre de choix de $p$ et de $q$ est fini. De plus, on a supposé le nombre d'éléments $\rho_\bullet\in\left<\mathcal{A}\mathcal{B}\right>$ fini, donc le nombre d'éléments qui s'écrivent \[ \rho_q\circ\rho_\bullet\circ\rho_p \] est fini. On a montré plus haut que tout élément de $\left<\mathcal{B}\mathcal{A}\right>$ sont de cette forme, on en conclut qu'il y en a un nombre fini.
...
On fait remarquer que cette proposition concerne bien la conjugaison, et ne s'étend par à $\mathcal{ABC}$ et $\mathcal{BAC}$, par exemple (voir figure \ref{fig:abc}).
...
\begin{figure}[h!]
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  \caption{Si on nomme $\mathcal{A}$ l'automate de gauche, $\mathcal{B}$ l'automate central et $\mathcal{C}$ l'automate de droite, alors $\mathcal{ABC}$ engendre un groupe fini, et $\mathcal{BAC}$ un groupe infini. \label{fig:abc}}
\end{figure}
...
Puisque nous nous intéressons à la classe des automates biréversibles, montrons qu'elle est bien close par les opérations qui nous intéressent, \emph{i.e.} la dualisation, la minimisation et la factorisation. Remarquons que les biréversibles ne sont pas clos par produit, alors que les inversibles et les réversibles (sous certaines conditions) le sont.
...
\begin{proof}[Démonstration de la proposition \ref{prop_cloture_inv_facteurs}]
  ...
  Or $\rho_{(p, r)}$ est une bijection donc $\rho'_r$ est surjective et $\rho''_p$ est injective (lemme \ref{lem:bij}). Or puisque ce sont des fonctions de $\Sigma$ dans $\Sigma$ qui est fini on a l'équivalence
...
La propriété \ref{prop:clot_birev} nous permet de connaître la structure des facteurs d'un automate biréversible, aidant ainsi à l'implémentation de l'algorithme suivant.
...
Les machines de Mealy sont représentées en mémoire par deux matrices \texttt{delta} et \texttt{rho}, l'une représentant les $(\delta_x)_{x\in\Sigma}$ et l'autre les $(\rho_p)_{p\in\mathcal{Q}}$. Dans les deux cas, les lignes des matrices sont indexées par les états et les colonnes par les lettres.

\begin{enumerate}[(i)]
\item On reconnaît une machine réversible en vérifiant que les colonnes de \texttt{delta}  sont des permutations.
\item On reconnaît une machine inversible en vérifiant que les lignes dans \texttt{rho} sont des permutations.
\end{enumerate}
...
Nous maintenons une liste de \texttt{sources} et de \texttt{cibles} qui représentent les sommets du graphe en hélice desquels encore aucune flèche ne part et les sommets du graphe en hélice vers lesquels encore aucune flèche ne pointe. En choisissant nos sommets dans ces ensembles nous nous assurons que le graphe en hélice obtenu est bien une union de cycles disjoints. À chaque fois que nous modifions \texttt{delta} ou \texttt{rho}, nous vérifions que la machine est bien inversible-réversible.
...
On a rapidement constaté que le nombre d'automates à engendrer est très important. Aussi, il ne semble pas réaliste de tester la $\mathfrak{mdc}$-réduction sur tous ceux-ci. On a donc cherché à extraire seulement les classes d'isomorphismes au sens de Nérode de notre génération.
...
On constate que l'utilisation de Nauty ralentit très significativement le temps d'exécution.
...
Comme dit dans la section \ref{sec:action}, la $\mathfrak{md}$-réduction résout le problème de finitude pour les automates biréversibles à deux états/lettres. La figure \ref{fig:fantastiques} montre, pour les biréversibles à 4 états et 3 lettres, les quatre automates qui ne se $\mathfrak{md}$-trivialisent pas, mais engendrent du fini. Ces automates se factorisent, et en permutant l'ordre de leurs facteurs, on trouve des automates $\mathfrak{md}$-triviaux.
...
Le calcul sur les biréversibles à 3 états et 3 lettres est très rapide et ne montre pas de contre-exemples. La plus petite classe d'automates biréversibles irréductibles est ensuite les automates à 5 états et 3 lettres. Nous en avions dénombrés 347 753 à isomorphisme près (table \ref{table:birev-iso}). Pour arriver au bout de cette classe d'automates, nous avons implémenté le calcul du $\mathfrak{md}$-réduit et de la fonction de croissance en C puis paralléliser son exécution. Grâce au super calculateur de l'université nous avons pu trouver 8 contre-exemples.
...
\subsection{Automates $\mathfrak{c}$-faible}

Nous avons ensuite cherché dans des classes plus grandes que celles abordées jusqu'alors pour trouver des automates \textit{$\mathfrak{c}$-faible}.

\begin{definition}
  Un automate est \textbf{\textit{$\mathfrak{c}$-faible}} s'il est $\mathfrak{mdc}$-trivial mais pas $\mathfrak{md}$-trivial.
\end{definition}

La factorisation est une opération coûteuse, pour tester la $\mathfrak{mdc}$-réduction il est donc préférable d'énumérer les classes de diviseurs et de faire tous les produits. On trouve ainsi tous les automates $\mathfrak{c}$-faible d'une classe à isomorphisme près.
...
\begin{table}[h!]
  \begin{center}
    \begin{threeparttable}
      \begin{tabular}{|rrr|}
        \hline
        \#états & \#lettres & \#$\mathfrak{c}$-faibles \\ [0.5ex]
        \hline\hline
        \hline
        4 & 3 & 4 \\
        \hline
        ...
        \hline
      \end{tabular}
      \caption{Nombre d'automates $\mathfrak{c}$-faibles pour des classes d'automates biréversibles.}
    \end{threeparttable}
  \end{center}
\end{table}
...
On pourrait se demander s'il y a des points communs entre les $\mathfrak{c}$-faible et les contre-exemples irréductibles que nous avons trouvé. Nous pourrions alors peut-être ajouter une opération à la $\mathfrak{mdc}$-réduction qui résolve le problème de finitude pour les groupes d'automates biréversibles.

...
Les automates de Mealy sont une extension des automates qui écrivent des mots en même temps qu'ils en lisent. À partir de l'action de ces automates sur un ensemble de mot, on peut dégager une structure de (semi-)groupe engendré par l'automate. Pour étudier cette classe de (semi-)groupes on peut travailler sur des constructions spécifiques à la structure sous jacente d'automate (comme la minimisation ou la dualisation) pour résoudre des problèmes d'algèbre de la théorie des groupes.

Le problème de théorie des groupes auquel nous nous intéressont ici le problème de finitude, c'est à dire savoir si un groupe engendré par un automate est fini ou non. C'est un problème difficile, indécidable en général pour les semi-groupes d'automates. Cependant Klimann\cite{Klimann13} a réussi à montrer que les groupes engendrés par des automates biréversibles à deux états ou deux lettres sont finis si et seulement si l'automate est $\mathfrak{md}$-trivial.

Avec M. Picantin, nous aimerions étendre ce résultat aux automates de plus de deux lettres mais il ne semble pas correct pour les automates à 4 états et 3 lettres (voir les 4 contres exemples figure \ref{fig:fantastiques}). Cepedant M. Picantin a remarqué que ces automates se factorisent et que l'automate obtenu en inversant les deux termes de la factorisation est quand à lui $\mathfrak{md}$-trivial. Nous nous sommes basé sur cet exemple pour fonder la conjecture \ref{conj:birev-mdc} qui a fait tout l'objet de notre recherche.

Pour tenter d'infirmer ou de se convaincre de la justesse de cette conjecture nous avons à la fois fait un travail théorique pour se convaincre de son bien fondé (sections \ref{sec:action} et \ref{sec:cloture}) et en même temps avons été amené à imaginer et implémenter des techniques efficaces pour générer les classes d'automates qui nous intéressent (section \ref{sec:gen}) et les factoriser (section \ref{sec:facto}) -- question sur laquelle aucun vrai résultat n'existe encore. À partir de ces résultats nous avons ensuite essayé de determiner si la $\mathfrak{mdc}$-réduction fournit un bon critère de finitude pour des classes d'automates plus grandes (section \ref{sec:finitude}).
...
\begin{center}
  \begin{figure}[h]
    ...
    \caption{Contre exemple à une génélisation du théorème \ref{thm:K}, aka Les 4 fantastiques. Le 4eme est l'inverse du premier.\label{fig:fantastiques}}
  \end{figure}
\end{center}
...
On a rapidement constaté que le nombre d'automates a générer est très important. Aussi, il ne semble pas réaliste de tester la $\mathfrak{mdc}$-réduction sur tout ceux-ci. On a donc cherché à extraire seulement les classes d'isomorphismes au sens de Nérode de notre génération.
...
\begin{figure}[h]
  \begin{center}
    \begin{tikzpicture}[scale=0.8]
      ...
      \node[draw,circle,minimum width = 2.5em] (QA) at (A) {$1|a$};
      \node[draw,circle,minimum width = 2.5em] (QB) at (B) {$2|a$};
      \node[draw,circle,minimum width = 2.5em] (QC) at (C) {$2|b$};
      \node[draw,circle,minimum width = 2.5em] (QD) at (D) {$1|b$};

      \node[draw,circle,minimum width = 2.5em,fill=black!20!green] (F1) at (P1) {$1$};
      \node[draw,circle,minimum width = 2.5em,fill=black!20!green] (F2) at (P2) {$2$};
...
\begin{table}[h!]
  \begin{center}
    \begin{threeparttable}
      \begin{tabular}{|rrrr|}
        \hline
        \#états & \#lettres & Temps d'execution & \#biréversibles \\ [0.5ex]
        \hline\hline
        2 & 2 & 0.002s & 12 \\
        \hline
        3 & 2 & 0.003s & 144 \\
...
On constate que Nauty ralenti très significativement le temps d'execution.
...
Une première manière de tester la conjecture \ref{conj:birev-mdc} est de tester si la $\mathfrak{md}$-réduction semble fonctionner dans des cas où la factorisation est impossible -- ie. quand le nombre d'état et de lettre est premier.
...
Nous manquons encore d’outils pour pouvoir affirmer qu’un groupe en-
gendre du fini ou non. Le problème étant indécidable en général cela
n’est pas surprenant. La prochaine étape serait de chercher des
critère de finitude analogue à la fonction de masse qui seraient plus efficace, ou qui permettraient de réduire le nombre d'automates à traiter. L’étude de la fonction de masse seule étant trop longue pour traiter des quantités d’automates comme celles que nous avons.

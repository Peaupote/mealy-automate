...
\begin{definition}
  \label{def:mdc-red}
  Une paire d'automates duaux est $\mathfrak{mdc}$-réduite si elle est $\mathfrak{md}$-réduite et que les deux automates ne sont irréductibles.
  La $\mathfrak{mdc}$-réduction consiste à répéter la $\mathfrak{md}$-réduction de l'automate et de ses conjugés jusqu'à touver des paires $\mathfrak{mdc}$-réduite.
\end{definition}

On remarque qu'il n'existe pas une unique $\mathfrak{mdc}$-réduction, contrairement à la $\mathfrak{md}$-réduction.

Ces contre-exemples sont pourtant factorisables, et en conjugant leur factorisation, on trouve un automate $\mathfrak{md}$-trivial. Ce qui conduit à la conjecture suivante.
...
\begin{center}
  ...
  \begin{tikzpicture}
  	...
  	\node[draw,circle,minimum width = 2.5em] (QA1) at (A1) {};
  	\node[draw,circle,minimum width = 2.5em] (QB1) at (B1) {};
  	\node[draw,circle,minimum width = 2.5em] (QC1) at (C1) {};
  	\node[draw,circle,minimum width = 2.5em] (QD1) at (D1) {};
  	\draw[->,>=latex] (QA1) to[bend right] node[midway, below]{$1|0$} (QB1);
  	\draw[->,>=latex] (QB1) to[bend right] node[midway, above]{$2|0$} (QA1);
  	\draw[->,>=latex] (QA1) to[bend right] node[midway, above right]{$0|1$} (QC1);
  	\draw[->,>=latex] (QC1) to[bend right] node[midway, below left]{$0|2$} (QA1);
  	\draw[->,>=latex] (QA1) to[bend right] node[midway, above left]{$2|2$} (QD1);
  	\draw[->,>=latex] (QD1) to[bend right] node[midway, below right]{$1|1$} (QA1);
  	\draw[->,>=latex] (QB1) to[bend right] node[midway, below]{$1|2$} (QC1);
  	\draw[->,>=latex] (QC1) to[bend left=40] node[midway, above]{$2|1$} (QB1);
  	\draw[->,>=latex] (QC1) to[bend right] node[midway, right]{$1|0$} (QD1);
  	\draw[->,>=latex] (QD1) to[bend left=40] node[midway, left]{$2|0$} (QC1);
  	\draw[->,>=latex] (QD1) to[bend right] node[midway, above]{$0|2$} (QB1);
  	\draw[->,>=latex] (QB1) to[bend left=40] node[midway, below]{$0|1$} (QD1);
  	...
  	\node[draw,circle,minimum width = 2.5em] (QA2) at (A2) {};
  	\node[draw,circle,minimum width = 2.5em] (QB2) at (B2) {};
  	\draw[->,>=latex] (QA2) to[out=70,in=110,looseness=6] node[midway, above]{$0|2$} (QA2);
  	\draw[->,>=latex] (QA2) to[bend right] node[midway, left]{\begin{tabular}{c} $1|1$ \\ $2|0$ \end{tabular}} (QB2);
  	\draw[->,>=latex] (QB2) to[out=250,in=290,looseness=6] node[midway, below]{$0|2$} (QB2);
  	\draw[->,>=latex] (QB2) to[bend right] node[midway, right]{\begin{tabular}{c} $1|0$ \\ $2|1$ \end{tabular}} (QA2);
  	
  	\draw (8,0) node {$\times$};
  	
  	\coordinate (A3) at (10,2);
  	\coordinate (B4) at (10,-2);
  	\node[draw,circle,minimum width = 2.5em] (QA3) at (A3) {};
  	\node[draw,circle,minimum width = 2.5em] (QB4) at (B4) {};
  	\draw[->,>=latex] (QA3) to[out=70,in=110,looseness=6] node[midway, above]{$1|0$} (QA3);
  	\draw[->,>=latex] (QA3) to[bend right] node[midway, left]{\begin{tabular}{c} $0|2$ \\ $2|1$ \end{tabular}} (QB4);
  	\draw[->,>=latex] (QB4) to[out=250,in=290,looseness=6] node[midway, below]{$1|0$} (QB4);
  	\draw[->,>=latex] (QB4) to[bend right] node[midway, right]{\begin{tabular}{c} $0|1$ \\ $2|2$ \end{tabular}} (QA3);
  	
  	% conjugaison
  	
  	\draw[->,>=latex] (8, -4) to node[midway, left] {$\mathfrak{c}$} (8, -5);
  	...
  	\node[draw,circle,minimum width = 2.5em] (QA2) at (A2) {$b$};
  	\node[draw,circle,minimum width = 2.5em] (QB2) at (B2) {$a$};
  	\node[draw,circle,minimum width = 2.5em] (QC2) at (C2) {$c$};
  	\node[draw,circle,minimum width = 2.5em] (QD2) at (D2) {$d$};
  	\draw[->,>=latex] (QA2) to[out=115,in=155,looseness=6] node[midway, above left]{$1|2$} (QA2);
  	\draw[->,>=latex] (QA2) to[bend left] node[midway, above]{\begin{tabular}{c} $0|1$ \\ $2|0$ \end{tabular}} (QC2);
  	\draw[->,>=latex] (QB2) to[out=205,in=245,looseness=6] node[midway, below left]{$1|2$} (QB2);
  	\draw[->,>=latex] (QB2) to[bend left] node[midway, above]{\begin{tabular}{c} $0|0$ \\ $2|1$ \end{tabular}} (QD2);
  	\draw[->,>=latex] (QC2) to[out=25,in=65,looseness=6] node[midway, above right]{$1|2$} (QC2);
  	\draw[->,>=latex] (QC2) to[bend left] node[midway, below]{\begin{tabular}{c} $0|1$ \\ $2|0$ \end{tabular}} (QA2);
  	\draw[->,>=latex] (QD2) to[out=295,in=335,looseness=6] node[midway, below right]{$1|2$} (QD2);
  	\draw[->,>=latex] (QD2) to[bend left] node[midway, below]{\begin{tabular}{c} $0|0$ \\ $2|1$ \end{tabular}} (QB2);
  	
  	%minization
  	\draw[->,>=latex] (0, -13) to node[midway, right] {$\mathfrak{m}$} (0, -14);
  	
  	%minimisé du conjugué
  	
  	\coordinate (A2) at (-2,-17);
  	\coordinate (B2) at (2,-17);
  	\node[draw,circle,minimum width = 2.5em] (QA2) at (A2) {$bc$};
  	\node[draw,circle,minimum width = 2.5em] (QB2) at (B2) {$ad$};
  	\draw[->,>=latex] (QA2) to[out=70,in=110,looseness=6] node[midway, above]{\begin{tabular}{c} $0|0$ \\ $1|2$ \\ $2|1$ \end{tabular}} (QA2);
  	\draw[->,>=latex] (QB2) to[out=70,in=110,looseness=6] node[midway, above]{\begin{tabular}{c} $0|1$ \\ $1|2$ \\ $2|0$ \end{tabular}} (QB2);
  	
  	%dualisation
  	
  	\draw[->,>=latex] (3.5, -16) to node[midway, above] {$\mathfrak{d}$} (4.5, -16);
  	
  	% dual du minimisé du conjugué
  	
  	\coordinate (A2) at (6.5,-17);
  	\coordinate (B2) at (9.5,-17);
  	\coordinate (C2) at ($(A2) + (60:3)$);
  	\node[draw,circle,minimum width = 2.5em] (QA2) at (A2) {$0$};
  	\node[draw,circle,minimum width = 2.5em] (QB2) at (B2) {$2$};
  	\node[draw,circle,minimum width = 2.5em] (QC2) at (C2) {$1$};
  	
  	\draw[->,>=latex] (QA2) to[out=190,in=230,looseness=6] node[midway, below left]{$ad|ad$} (QA2);
  	\draw[->,>=latex] (QA2) to node[midway, above left] {$bc|bc$} (QC2);
  	\draw[->,>=latex] (QB2) to node[midway, below] {$bc|bc$} (QA2);
  	\draw[->,>=latex] (QB2) to[bend left] node[midway, below left] {$ad|ad$} (QC2);
  	\draw[->,>=latex] (QC2) to[bend left] node[midway, above right] {\begin{tabular}{c} $ad|ad$ \\ $bc|bc$ \end{tabular}} (QB2);
  	
  	%minimisation
  	\draw[->,>=latex] (8, -19) to node[midway, right] {$\mathfrak{m}$} (8, -20);
  	
  	\coordinate (A2) at (8, -23);
  	\node[draw,circle,minimum width = 2.5em] (QA2) at (A2) {$012$};
  	\draw[->,>=latex] (QA2) to[out=70,in=110,looseness=6] node[midway, above]{\begin{tabular}{c} $ad|ad$ \\ $bc|bc$ \end{tabular}} (QA2);
  	
  \end{tikzpicture}
  }
\end{center}
  \caption{$\mathfrak{mdc}$-réduction du deuxième contre-exemple}
\end{figure}
...
Nous avons donc décidé d'implémenter une version C beaucoup plus efficace. Nous avons pu augmenter l'efficacité de notre programme en le parallélisant.
